-- Use one focused RPC instead of direct browser insertion so task creation
...
create function public.create_assigned_task(
...
begin
  if caller_id is null then
    raise exception using
      errcode = '28000',
      message = 'Authentication is required to create a task.';
  end if;
  ...
  if caller_role is distinct from 'manager'::public.app_role then
    raise exception using
      errcode = '42501',
      message = 'Only a manager may create an assigned task.';
  end if;

  if caller_team_id is null then
    raise exception using
      errcode = '42501',
      message = 'A manager team assignment is required to create a task.';
  end if;
  ...
  if p_due_date is null then
    raise exception using
      errcode = '22004',
      message = 'A task due date is required.';
  end if;

  if p_priority is null then
    raise exception using
      errcode = '22004',
      message = 'A task priority is required.';
  end if;
  ...
  if p_related_customer_id is not null
    and not exists (
      ...
      where customer.id = p_related_customer_id
        and customer.team_id = caller_team_id
    )
  then
    raise exception using
      errcode = '42501',
      message = 'Related customer was not found in the authenticated manager team.';
  end if;

  insert into public.tasks as inserted_task (
    team_id,
    related_customer_id,
  ...
  returning inserted_task.id, inserted_task.created_at
  into inserted_task_id, inserted_created_at;

  return query
  select inserted_task_id, inserted_created_at;
end;
$$;

comment on function public.create_assigned_task(
  uuid,
  text,
  date,
  ...
  'Creates one open task for a manager-owned team by deriving team and creator identity from auth.uid() while preserving RLS through SECURITY INVOKER.';

revoke all on function public.create_assigned_task(
  uuid,
  text,
  date,
  public.priority_level,
