-- Use one focused RPC instead of a direct browser table update so callers can
...
create function public.complete_assigned_follow_up(
  p_follow_up_id uuid,
...
begin
  if caller_id is null then
    raise exception using
      errcode = '28000',
      message = 'Authentication is required to complete a follow-up.';
  end if;
  ...
  if caller_role is distinct from 'sales_executive'::public.app_role then
    raise exception using
      errcode = '42501',
      message = 'Only an assigned sales executive may complete a follow-up.';
  end if;

  if p_follow_up_id is null then
    raise exception using
      errcode = '22004',
      message = 'A follow-up ID is required.';
  end if;
  ...
  select
    follow_up.assigned_sales_executive_id,
    follow_up.state
  into
  ...
  if not found then
    raise exception using
      errcode = '42501',
      message = 'Follow-up was not found or is not assigned to the authenticated user.';
  end if;

  if locked_assigned_sales_executive_id is distinct from caller_id then
    raise exception using
      errcode = '42501',
      message = 'The authenticated user is not assigned to this follow-up.';
  end if;
  ...
  if locked_state is distinct from 'open'::public.work_item_state then
    raise exception using
      errcode = '55000',
      message = format(
        'Only an open follow-up can be completed; current state is %s.',
        locked_state
      );
  end if;
  ...
  where follow_up.id = p_follow_up_id
    and follow_up.assigned_sales_executive_id = caller_id
    and follow_up.state = 'open'::public.work_item_state
  returning follow_up.completed_at
  ...
  if affected_rows <> 1 then
    raise exception using
      errcode = '55000',
      message = 'The open follow-up could not be marked as completed.';
  end if;

  return query
  select p_follow_up_id, updated_completed_at;
end;
$$;

comment on function public.complete_assigned_follow_up(uuid, text) is
  'Completes one open follow-up assigned to the authenticated sales executive while preserving RLS through SECURITY INVOKER.';

revoke all on function public.complete_assigned_follow_up(uuid, text)
  from public, anon, authenticated;

grant execute on function public.complete_assigned_follow_up(uuid, text)
  to authenticated;
