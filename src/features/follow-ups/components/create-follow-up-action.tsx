...
interface CreateFollowUpActionProps {
  isManager: boolean;
  options: FollowUpCreationOptions | null;
  defaultDate: string;
}
...
const createFollowUpErrorMessage =
  "We could not create this follow-up. Refresh the page and try again.";
...
export function CreateFollowUpAction({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const { error } = await supabase.rpc("create_assigned_follow_up", {
    ...
    setIsSaving(false);
    setSuccessMessage(
      "Follow-up created. The live queue and summary counts have been refreshed.",
    );
    router.refresh();
  }
  ...
  return (
    <section
      aria-labelledby="create-follow-up-action-title"
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h2
            id="create-follow-up-action-title"
            className="mt-1 text-base font-semibold text-slate-950"
          >
            Create and assign a follow-up
          </h2>
          <p className="mt-1 max-w-2xl text-sm leading-6 text-blue-900/80">
            Add an open follow-up using authorized team customers and sales
            executives.
          </p>
        </div>
        ...
        >
          {isOpen ? "Close creator" : "Create follow-up"}
        </button>
      </div>
        ...
        canSubmit ? (
          <form onSubmit={handleSubmit} className="mt-5 grid gap-4 sm:grid-cols-2">
            <fieldset disabled={isSaving} className="contents">
              ...
              <div className="flex flex-col-reverse gap-2 sm:col-span-2 sm:flex-row sm:justify-end">
                ...
                >
                  {isSaving ? "Creating follow-up..." : "Create open follow-up"}
                </button>
              </div>
            </fieldset>
          </form>
          ...
          >
            ...
            <p className="mx-auto mt-1 max-w-xl text-xs leading-5 text-slate-500">
              ...
              authorized sales executive before a follow-up can be created.
            </p>
          </div>
        )
      ) : null}
