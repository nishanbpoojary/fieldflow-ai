...
export function TaskForm({ customers, defaultDate, onCreate }: TaskFormProps) {
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!title || !customerId || !dueDate || !isTaskPriority(priority)) {
      return;
    }
  ...
  return (
    ...
    >
      ...
      <h2 id="create-task-title" className="mt-1 text-lg font-semibold text-slate-950">
        Create a task
      </h2>
      <p className="mt-1 text-sm leading-6 text-slate-500">
        Add work for one of Maya Chen&apos;s assigned customers.
      </p>
      ...
      {customers.length > 0 ? (
        <form onSubmit={handleSubmit} className="mt-5 grid gap-4 sm:grid-cols-2">
          <label className="text-sm font-medium text-slate-700 sm:col-span-2">
            ...
            <input
              ...
              type="text"
              required
              placeholder="For example, prepare account review"
          ...
          <label className="text-sm font-medium text-slate-700 sm:col-span-2">
            ...
            <textarea
              name="planningNote"
              rows={3}
              placeholder="Add useful context for the task"
        ...
        <div className="mt-5 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center">
          <p className="text-sm font-semibold text-slate-700">No assigned customers</p>
