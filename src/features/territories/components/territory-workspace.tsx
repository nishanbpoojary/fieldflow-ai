...
import type {
  TerritoryMetric,
  TerritoryPageContext,
  TerritorySummary,
  TerritoryWorkspaceData,
  TerritoryWorkspaceResult,
      ...
      <main className="min-w-0 flex-1 px-4 py-6 sm:px-6 lg:px-8 lg:py-8 xl:px-10">
        <div className="mx-auto max-w-7xl">
          <header className="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
                A focused view of territory coverage, current-week visits,
                at-risk customers, and customer-linked overdue work across your
                authorized team.
              </p>
            </div>
          ...
          {result.status === "unavailable" ? (
            <TerritoryState
              title="Territory metrics are temporarily unavailable"
              description="We could not load the authorized territory metrics right now. Please refresh the page or try again shortly."
              tone="warning"
            />
          ) : result.data.territories.length === 0 ? (
            <TerritoryState
              title="No territories found"
              description="This team does not currently have territory records to summarize."
              tone="empty"
            />
          ) : (
            <TerritoryContent data={result.data} />
          )}
        </div>
      </main>
    </div>
      ...
      <main className="min-w-0 flex-1 px-4 py-6 sm:px-6 lg:px-8 lg:py-8 xl:px-10">
        <section className="mx-auto max-w-3xl rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm shadow-slate-200/60">
          ...
          <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-500">
            Territories contains team-wide coverage analytics and is available
            only to authenticated Managers. Your personal workspace remains
...
function TerritoryContent({ data }: { data: TerritoryWorkspaceData }) {
  return (
    <div className="mt-6 space-y-6">
      <section aria-labelledby="territory-summary-title">
        <div className="mb-4">
          <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-blue-700">
            Territory summary
          </p>
          <h2
            id="territory-summary-title"
            className="mt-1 text-lg font-semibold tracking-tight text-slate-950"
          >
            Current-week territory health
          </h2>
        </div>
        ...
        <div className="flex flex-col gap-3 border-b border-slate-100 pb-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-500">
              Metrics are attributed through customer-territory relationships.
              Internal tasks without linked customers are intentionally excluded.
            </p>
          </div>
          <span className="w-fit rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-600">
            {data.territories.length}{" "}
            {data.territories.length === 1 ? "territory" : "territories"}
          </span>
        </div>
...
function SummaryGrid({ summary }: { summary: TerritorySummary }) {
  const cards = [
    {
      label: "Covered Territories",
      value: summary.territoriesWithCustomers.toLocaleString(),
      detail: "Territories with at least one assigned customer",
    },
    {
      label: "Assigned Customers",
      value: summary.assignedCustomers.toLocaleString(),
      detail: "Customer records tied to authorized territories",
    },
    {
    ...
    },
    {
      ...
      detail: "Completed visits attributed through customers",
    },
    {
    ...
    },
    {
      ...
      detail: "Assigned customers marked at risk",
    },
  ];
...
function TerritoryCard({ territory }: { territory: TerritoryMetric }) {
  return (
    <article className="rounded-xl border border-slate-200 bg-slate-50 p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm leading-6 text-slate-500">
            {territory.coveredExecutiveCount}{" "}
            {territory.coveredExecutiveCount === 1
              ? "Sales Executive"
              : "Sales Executives"}{" "}
            covering assigned customers
          </p>
        </div>
        <span className="w-fit rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700">
          {territory.completionRate}% complete
        </span>
      </div>
      ...
      <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-200">
        <div
          ...
          style={{ width: `${Math.min(territory.completionRate, 100)}%` }}
        />
      </div>

      <dl className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">
        <Metric label="Customers" value={territory.assignedCustomerCount} />
        <Metric label="Active" value={territory.activeCustomerCount} />
        <Metric label="At-risk" value={territory.atRiskCustomerCount} />
        <Metric label="Planned" value={territory.plannedVisits} />
        <Metric label="Completed" value={territory.completedVisits} />
        <Metric
          label="Overdue work"
          value={territory.overdueFollowUps + territory.overdueTasks}
        />
      </dl>
      <p className="mt-3 text-xs leading-5 text-slate-500">
        Overdue work includes {territory.overdueFollowUps} follow-up
        {territory.overdueFollowUps === 1 ? "" : "s"} and{" "}
        {territory.overdueTasks} customer-linked task
        {territory.overdueTasks === 1 ? "" : "s"}.
      </p>
    </article>
  );
}
