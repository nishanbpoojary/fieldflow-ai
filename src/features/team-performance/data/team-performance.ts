...
import {
  ...
  type TeamPerformanceCustomerInput,
  type TeamPerformanceExecutiveInput,
  type TeamPerformanceTerritoryInput,
  type TeamPerformanceVisitInput,
  type TeamPerformanceVisitPlanInput,
  type TeamPerformanceWorkItemInput,
} from "@/features/team-performance/data/team-performance-rules";
import type { TeamPerformanceResult } from "@/features/team-performance/types";
...
export async function getTeamPerformanceData(
  ...
  const executives: TeamPerformanceExecutiveInput[] = profilesResult.data.map(
    (profile) => ({
      id: profile.id,
  ...
  const territories: TeamPerformanceTerritoryInput[] =
    territoriesResult.data.map((territory) => ({
      id: territory.id,
      name: territory.name,
    }));

  const customers: TeamPerformanceCustomerInput[] = customersResult.data.map(
    (customer) => ({
      id: customer.id,
  ...
  const visitPlans: TeamPerformanceVisitPlanInput[] =
    visitPlansResult.data.map((plan) => ({
  ...
  const visits: TeamPerformanceVisitInput[] = visitsResult.data.map((visit) => ({
  ...
  const followUps: TeamPerformanceWorkItemInput[] = followUpsResult.data.map(
    (followUp) => ({
  ...
  const tasks: TeamPerformanceWorkItemInput[] = tasksResult.data.map((task) => ({
