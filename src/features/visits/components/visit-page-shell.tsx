          ...
          <header className="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h1 className="mt-3 text-2xl font-semibold tracking-tight text-slate-950 sm:text-3xl">
                {isManager ? "Team Visit Planner" : "Today’s Visit Planner"}
              </h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
                {isManager
                  ? "Plan synthetic team visits and review visit status across the demo territories."
                  : "Plan Maya Chen’s customer visits and capture outcomes for pending visits."}
              </p>
            </div>
            <p className="text-sm font-medium text-slate-600">{formatOverviewDate(demoToday)}</p>
          </header>

          <aside className="my-6 rounded-xl border border-blue-100 bg-blue-50/70 px-4 py-3 text-sm leading-6 text-blue-900" aria-label="Demo reset notice">
            Changes are session-only demo state. Planning or completing a visit is reset when this page is refreshed.
          </aside>
